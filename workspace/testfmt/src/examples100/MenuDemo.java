...
/**
 * Title: 菜单演示 Description: 演示菜单的建立和快捷键的使用。 Filename: MenuDemo.java
 */
...
public class MenuDemo implements ActionListener, ItemListener {
	...
	public JMenuBar createMenuBar() {
		...
		menu = new JMenu("(N)第三个菜单");
		menu.setMnemonic(KeyEvent.KEY_FIRST);
		menuBar.add(menu);
	...
	/**
	 * 方法说明：监听普通的菜单选择 输入参数：ActionEvent e 事件 返回类型：
	 */
	public void actionPerformed(ActionEvent e) {
		JMenuItem source = (JMenuItem) (e.getSource());
		String s = "监测事件。" + newline + "    事件源: " + source.getText()
				+ " (选择对象" + getClassName(source) + ")";
		output.append(s + newline);
	}

	/**
	 * 方法说明：监听检查盒菜单选择项 输入参数：ItemEvent e 检查盒触发的事件 返回类型：
	 */
	public void itemStateChanged(ItemEvent e) {
		JMenuItem source = (JMenuItem) (e.getSource());
		String s = "菜单项监听" + newline + "    事件源: " + source.getText()
				+ " (选择对象 " + getClassName(source) + ")" + newline
				+ "    新的状态: "
				+ ((e.getStateChange() == ItemEvent.SELECTED) ? "选择" : "不选择");
		output.append(s + newline);
	}

	/**
	 * 方法说明：获得类的名字 输入参数： 返回类型：
	 */
	...
	/**
	 * 方法说明：根据路径查找图片 输入参数：String path 图片的路径 返回类型：ImageIcon 图片对象
	 */

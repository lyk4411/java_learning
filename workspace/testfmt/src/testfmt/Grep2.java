...
/**
 * A command-line grep-like program. Accepts some command-line options, and
 * takes a pattern and a list of text files.
 */
...
public class Grep2 {
	...
	/**
	 * Construct a Grep2 object for each pattern, and run it on all input files
	...
	public static void main(String[] argv) {
		if (argv.length < 1) {
			System.err
					.println("Usage: Grep2 pattern [-chilsnv][-f pattfile]"
							+ "[filename...]");
			System.exit(1);
		}
		String patt = null;
		...
		while ((c = go.getopt(argv)) != 0) {
			...
			case 'f':
				try {
					...
					System.err
							.println("Can't read pattern file " + go.optarg());
					System.exit(1);
				}
				break;
			case 'h':
	...
	public Grep2(String patt, BitSet args) throws PatternSyntaxException {
		...
		if (args.get('V'))
			inVert = true;
		int caseMode = ignoreCase ? Pattern.UNICODE_CASE
	 ...
	 * 
	 * @param ifile
	 *            Reader Reader object already open
	 * @param fileName

...
public class Sprite extends Component implements Runnable {
	...
	public void start() {
		t = new Thread(this);
		//t.setName("Sprite    aaaa #" + ++spriteNumber);
		t.start();
	}
	...
	/**
	 * Run one Sprite around the screen. This version just moves them around
	 * either across, down, or at some 45-degree angle.
	 */
	public void run() {
		...
		int xincr = Math.random() > 0.5 ? 5 : -5;
		int yincr = Math.random() > 0.5 ? 5 : -5;
		while (!done) {

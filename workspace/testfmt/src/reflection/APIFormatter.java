...
/**
 * <p>
 * APIFormatter reads one or more Zip files, gets all entries from each and, for
 * each entry that ends in ".class", loads it with Class.forName() and hands it
 * off to a doClass(Class c) method declared in a subclass. <br/>
 * TODO<br/>
...
public abstract class APIFormatter {
	...
	protected int doArgs(String[] argv) throws IOException {
		...
		if (argv.length == 0) {
			...
			while (st.hasMoreTokens()) {
				...
				System.out.println("Trying path " + thisFile);
	...
	public void processOneZip(String fileName) throws IOException {
		...
		Enumeration<? extends ZipEntry> all = (Enumeration<? extends ZipEntry>) zipFile
				.entries();
		...
		for (ZipEntry zipEntry : entries) {
			...
			// java.lang.Math
			String className = zipName.replace('/', '.').substring(0,
					zipName.length() - 6); // 6 for ".class"
			...
			try {
				...
				System.err.println("aaaaaaaa: " + className.toString());
			}
	...
	/**
	 * Template Method to do something useful (e.g., format the fields and

...
// BEGIN main
/**
 * Subclass of LineNumberReader, parent of others, to allow reading of continued
 * lines using the readLine() method. The other Reader methods (readInt()) etc.)
 * must not be used. Must subclass to provide the actual implementation of
 * readLine().
 */
public abstract class ContLineReader extends LineNumberReader {
	...
	/**
	 * Read one (possibly continued) line, stripping out the \ that marks the
	 * end of each line but the last in a sequence.
	 */
	public abstract String readLine() throws IOException;

	/**
	 * Read one real line. Provided as a convenience for the subclasses, so they
	 * don't embarrass themselves trying to call "super.readLine()" which isn't
	 * very practical...
	 */
